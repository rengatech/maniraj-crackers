...
const onOrderPage = computed(() => route().current('order-now'))
...
<template>
    <header :class="['sticky top-0 z-50 bg-white transition-shadow duration-300', scrolled ? 'shadow-md' : '']">
        <div class="mx-auto max-w-6xl px-4">
            <div class="flex h-20 min-w-0 items-center justify-between gap-2">
                <!-- Logo -->
                <Link href="/" class="flex min-w-0 items-center gap-1.5 sm:gap-2">
    ...
    <span class="truncate font-bold text-[11px] tracking-wide whitespace-nowrap sm:text-xl sm:tracking-wider" style="font-family: 'Cinzel', serif; color: #b91c1c;">MANIRAJ CRACKERS</span>
    <span class="hidden sm:block text-[9px]" style="font-family: 'Poppins', sans-serif; color: #1d4ed8;">Best Quality Fireworks Online @ Affordable Price</span>
                ...
                <nav class="hidden items-center gap-6 md:flex lg:gap-8">
                    ...
                    <Link
                        href="/order-now"
                        :class="[
                            'order-now-cta relative inline-flex items-center gap-2 rounded-full px-5 py-2 text-sm font-bold uppercase tracking-wider text-white shadow-lg transition-transform hover:scale-105',
                            onOrderPage ? 'ring-2 ring-amber-300 ring-offset-2' : 'animate-cta-glow',
                        ]"
                    >
                ...
                <div class="flex shrink-0 items-center gap-1 sm:gap-3">
                    ...
                    <button
                        ...
                        class="hidden items-center gap-1.5 rounded-lg border-2 border-brand-red px-2.5 py-1.5 text-xs font-semibold text-brand-red transition-colors hover:bg-brand-red hover:text-white sm:flex sm:px-4 sm:text-sm"
                    >
                    ...
                    <!-- Mobile Order Now CTA (hidden on the order page itself, where the cart icon takes its place) -->
                    <Link
                        v-if="!onOrderPage"
                        href="/order-now"
                        class="order-now-cta animate-cta-glow relative inline-flex items-center gap-1 rounded-full px-3 py-1.5 text-[11px] font-bold uppercase tracking-wide text-white shadow-md md:hidden"
                    >
                        ...
                        <span
                            v-if="totalItems > 0"
                            class="absolute -right-1.5 -top-1.5 flex h-5 min-w-5 items-center justify-center rounded-full bg-amber-400 px-1 text-[10px] font-bold text-brand-dark ring-2 ring-white"
                        >
                            {{ totalItems }}
                        </span>
                    </Link>
                    ...
                    <button
                        ...
                        :class="['relative rounded-lg p-1.5 text-brand-dark transition-colors hover:bg-gray-100 sm:p-2', onOrderPage ? '' : 'hidden md:block', cartBounce ? 'animate-cart-wiggle' : '']"
                    >
                    ...
                    <button
                        ...
                        class="rounded-lg p-1.5 text-brand-dark md:hidden"
                    >
            ...
            <div v-if="mobileMenuOpen" class="absolute left-0 right-0 top-20 z-50 bg-white shadow-lg md:hidden">
                <div class="space-y-1 px-4 pb-4 pt-2">
                    <Link
                        ...
                        class="order-now-cta mb-2 flex items-center justify-center gap-2 rounded-xl px-3 py-3 text-base font-bold uppercase tracking-wider text-white shadow-md"
                    >
